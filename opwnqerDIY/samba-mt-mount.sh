...
# 确保samba4已经安装（建议自定义固件时勾选）
# 添加samba用户root，密码为root
smbpasswd_file="/etc/samba/smbpasswd"
if ! grep -q "^root:" "$smbpasswd_file" 2>/dev/null; then
  (echo "$root_password"; echo "$root_password") | smbpasswd -a root -s
fi

# 配置U盘共享（假设自动挂载点为/mnt/sda1）
smb_share_section="share_sda1"
if ! uci show samba4 | grep -q "$smb_share_section"; then
